...
static inline bool finite_pt(const idet::Point2f& p) noexcept {
    return std::isfinite(p.x) && std::isfinite(p.y);
}

static inline int count_finite(const idet::Point2f q[4]) noexcept {
    int c = 0;
    for (int i = 0; i < 4; ++i)
        c += finite_pt(q[i]) ? 1 : 0;
    return c;
...
static inline int count_nan_any(const idet::Point2f q[4]) noexcept {
    int c = 0;
    for (int i = 0; i < 4; ++i) {
...
static inline int count_inf_any(const idet::Point2f q[4]) noexcept {
    int c = 0;
    for (int i = 0; i < 4; ++i) {
...
static inline bool lex_yx_less(const idet::Point2f& a, const idet::Point2f& b, float eps = 1e-4f) noexcept {
...
static inline void expect_tl_is_lex_min(const idet::Point2f q[4]) {
...
static inline idet::Quad make_rect(float x0, float y0, float x1, float y1) {
    // {TL, TR, BR, BL}
    return {{{x0, y0}, {x1, y0}, {x1, y1}, {x0, y1}}};
}

static inline void shuffle_quad(idet::Point2f q[4], std::uint32_t seed = 123) {
    std::array<int, 4> idx = {0, 1, 2, 3};
    std::mt19937 rng(seed);
    std::shuffle(idx.begin(), idx.end(), rng);

    idet::Point2f tmp[4] = {q[idx[0]], q[idx[1]], q[idx[2]], q[idx[3]]};
    for (int i = 0; i < 4; ++i)
        q[i] = tmp[i];
}

static inline void expect_is_tl_tr_br_bl_rect(const idet::Point2f q[4]) {
...
static inline void copy_quad(idet::Point2f dst[4], const idet::Point2f src[4]) {
    for (int i = 0; i < 4; ++i)
        dst[i] = src[i];
}

static inline std::multiset<std::pair<float, float>> as_multiset(const idet::Point2f q[4]) {
...
static inline idet::Quad to_array(const idet::Point2f q[4]) {
...
static idet::Quad random_convex_quad(std::mt19937& rng) {
    ...
    const idet::Point2f c{center_d(rng), center_d(rng)};
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        q[i] = {c.x + r * std::cos(a[i]), c.y + r * std::sin(a[i])};
    }
    ...
    idet::Quad out = {q[idx[0]], q[idx[1]], q[idx[2]], q[idx[3]]};
    return out;
}
...
TEST(Geometry, OrderQuad_ThenQuadIou_SameShapeDifferentPermutation_IsOne) {
    ...
    idet::Point2f q1[4] = {base[0], base[1], base[2], base[3]};
    idet::Point2f q2[4] = {base[0], base[1], base[2], base[3]};
...
TEST(Geometry, OrderQuad_Rect_ShuffledManyTimes_IsStable) {
    ...
    for (std::uint32_t seed = 1; seed <= 80; ++seed) {
        idet::Point2f q[4] = {r[0], r[1], r[2], r[3]};
        shuffle_quad(q, seed);
...
TEST(Geometry, AabbIou_DegenerateZeroArea_ReturnsZero) {
    idet::Quad A = {{{1.f, 1.f}, {1.f, 1.f}, {1.f, 1.f}, {1.f, 1.f}}};
...
TEST(Geometry, QuadIou_UnorderedPoints_StillInRange) {
    idet::Quad A = {{{0, 0}, {10, 10}, {0, 10}, {10, 0}}};
    idet::Quad B = {{{0, 0}, {10, 0}, {10, 10}, {0, 10}}};
...
TEST(Geometry, QuadIou_SymmetricProperty_Holds) {
    idet::Quad A = {{{30.f, 10.f}, {80.f, 25.f}, {70.f, 70.f}, {20.f, 55.f}}};
    idet::Quad B = {{{40.f, 15.f}, {90.f, 30.f}, {75.f, 75.f}, {25.f, 60.f}}};
